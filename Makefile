...
docker-alma-gcc: go$(VERSION).linux-amd64.tar.gz 
	@docker build --progress=plain --no-cache --network=bridge --build-arg GOLANG_VERSION=$(VERSION) --build-arg ALMA_VERSION=8.8 -t golang-alma-gcc -f Dockerfile-alma-gcc .

.phony: docker-alma-clang
docker-alma-clang: go$(VERSION).linux-amd64.tar.gz 
	@docker build --progress=plain --no-cache --network=bridge --build-arg GOLANG_VERSION=$(VERSION) --build-arg ALMA_VERSION=8.8 -t golang-alma-clang -f Dockerfile-alma-clang .
